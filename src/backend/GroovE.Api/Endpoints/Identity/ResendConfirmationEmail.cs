...
public class ResendConfirmationEmail : IEndpoint
{
    ...
    public static async Task Handle([FromServices] IMediator mediator, [FromBody] ResendConfirmationEmailRequest request) => await mediator.Send(new ResendConfirmationEmailCommand(request.Email));
}
